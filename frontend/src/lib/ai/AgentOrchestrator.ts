...
type OrchestratorContext = {
  ...
  preferredExecutionMode?: 'instant' | 'think';
  project?: {
    id?: string;
    name?: string;
    placeId?: string;
    ...
    selectedNodeClassName?: string | null;
    selectedNodeSummary?: string | null;
    selectedScriptSummary?: string | null;
    workspaceNodes?: any[];
  };
...
type ResponseType = 'conversation' | 'analysis' | 'proposal';
type UsageTelemetry = {
  inputTokens: number;
  outputTokens: number;
  totalTokens: number;
...
type ExecutableCommand = {
  action: 'CreatePart' | 'CreateScript' | 'InsertAsset' | 'CreateInstance' | 'SetInstanceProperty' | 'RunLuau';
...
type StructuredResponse = {
  message: string;
  executions: StructuredExecution[];
};
type PluginCallExecution = {
  type: 'plugin_call';
  api:
...
type ExplorerSelection = {
  needExplorer: boolean;
  includeAll: boolean;
  includePaths: string[];
  includeClasses: string[];
  includeChildrenDepth: number;
  includeParentChain: boolean;
  includeScriptSource: boolean;
  reason: string;
};
...
export class AgentOrchestrator {
  static async processUserIntent(intent: string, context: OrchestratorContext) {
    ...
    const telemetryStages: UsageTelemetry['modelBreakdown'] = [];
  ...
  private static classifyIntent(intent: string) {
    ...
    const isGreeting =
      /^(oi|ola|olá|e ai|e aí|bom dia|boa tarde|boa noite|hello|hi|hey)\b/.test(normalized.trim());
    ...
    const responseType: ResponseType = isShortConversation
      ? 'conversation'
      : hasBuildIntent
        ? 'proposal'
  ...
  private static buildCompactContext(
    intent: string,
    context: OrchestratorContext,
    intentProfile: ReturnType<typeof AgentOrchestrator.classifyIntent>,
  ...
  private static selectModels(intentProfile: ReturnType<typeof AgentOrchestrator.classifyIntent>, preferredModel?: string) {
    return {
  ...
  private static buildGeneratorPrompt(
    intent: string,
  ...
  private static async runThinkSelection(
    intent: string,
    context: OrchestratorContext,
    intentProfile: ReturnType<typeof AgentOrchestrator.classifyIntent>,
    thinkModel: string
  ...
  private static buildReviewPrompt(
    ...
    return [
      `Pedido do usuario: ${intent}`,
      `Tipo: ${intentProfile.taskType}`,
      `Risco: ${intentProfile.riskLevel}`,
      PLUGIN_DOC_CAPSULE,
      `Conteudo gerado: ${this.limit(generatedContent, 1800)}`,
      'Revise seguranca, consistencia, escopo e riscos. Aponte bloqueios reais e ajustes objetivos.',
      compactContext.summary,
    ].join('\n\n');
  }

  private static async rethinkAndRepairResponse(
    intent: string,
    compactContext: { summary: string },
    intentProfile: ReturnType<typeof AgentOrchestrator.classifyIntent>,
    ...
    reviewModel: string,
    telemetryStages: UsageTelemetry['modelBreakdown'],
    forceRepairReason = ''
  ...
  private static async runPreflightValidation(
    intent: string,
    compactContext: { summary: string },
    intentProfile: ReturnType<typeof AgentOrchestrator.classifyIntent>,
    structuredResponse: StructuredResponse,
  ...
  private static pushUsageTelemetry(
    target: UsageTelemetry['modelBreakdown'],
    stage: string,
    response: any
  ) {
  ...
  private static finalizeUsageTelemetry(modelBreakdown: UsageTelemetry['modelBreakdown']): UsageTelemetry {
    return {
  ...
  private static extractAttachmentSummary(intent: string) {
    const matches = [...intent.matchAll(/\('((?:\\.|[^'])*)',\s*"((?:\\.|[^"])*)"\)/g)];
  ...
  private static limit(value: string, max = 240) {
    const normalized = String(value || '').replace(/\s+/g, ' ').trim();
    if (normalized.length <= max) return normalized;
    return `${normalized.slice(0, max - 3)}...`;
  }

  private static parseExplorerSelection(rawText: string, selectedPath: string | null): ExplorerSelection {
    const fallback: ExplorerSelection = {
    ...
    try {
      ...
      return {
        ...
        includePaths: Array.isArray(parsed?.includePaths)
          ? parsed.includePaths.filter((item: any) => typeof item === 'string' && item.trim()).slice(0, 12)
          : fallback.includePaths,
        includeClasses: Array.isArray(parsed?.includeClasses)
          ? parsed.includeClasses.filter((item: any) => typeof item === 'string' && item.trim()).slice(0, 8)
          : [],
  ...
  private static buildFilteredExplorerContext(
    nodes: any[],
    ...
    const desiredPaths = new Set<string>(selection.includePaths || []);
    if (selectedPath) desiredPaths.add(selectedPath);

    const relevantNodes = selection.includeAll
      ? nodes.slice(0, 20)
      : this.collectRelevantNodes(nodes, desiredPaths, new Set(selection.includeClasses || []), selection.includeChildrenDepth, selection.includeParentChain);
  ...
  private static collectRelevantNodes(
    nodes: any[],
    desiredPaths: Set<string>,
    desiredClasses: Set<string>,
    includeChildrenDepth: number,
    includeParentChain: boolean
  ) {
    const results = new Map<string, any>();

    const walk = (node: any, parents: any[]) => {
  ...
  private static collectChildren(node: any, depth: number, results: Map<string, any>) {
    if (depth <= 0) return;
    for (const child of Array.isArray(node?.filhos) ? node.filhos : []) {
  ...
  private static serializeNodeForPrompt(node: any, includeScriptSource: boolean) {
  ...
  private static tryBuildLocalExecutionPlan(
    intent: string,
    context: OrchestratorContext,
    intentProfile: ReturnType<typeof AgentOrchestrator.classifyIntent>
  ): { plan: string; reply: string; executableCommands: ExecutableCommand[]; executions: StructuredExecution[] } | null {
  ...
  private static buildSinglePartPlan(shape: 'Block' | 'Ball' | 'Cylinder', reply: string) {
    const source = [
  ...
  private static buildPyramidPlan(_targetPath: string) {
    const source = [
  ...
  private static buildStreetScenePlan() {
    const source = [
      'local created = {}',
  ...
  private static buildStructuredResponse(message: string, executions: StructuredExecution[]): StructuredResponse {
    return { message, executions };
  }

  private static ensureExecutionFallback(
  ...
  private static parseStructuredResponse(rawText: string): StructuredResponse {
      ...
      if (parsed && typeof parsed.message === 'string' && Array.isArray(parsed.executions)) {
        const structured = {
          ...
          executions: parsed.executions
            .filter((item: any) => typeof item?.executionId === 'number' && typeof item?.source === 'string')
            .map((item: any) => ({
  ...
  private static detectMalformedStructuredEnvelope(rawText: string) {
  ...
  private static extractJsonObject(rawText: string) {
    const trimmed = String(rawText || '').trim();
  ...
  private static extractExecutableSourceCandidate(rawText: string) {
    const trimmed = String(rawText || '').trim();
    if (!trimmed) return '';
  ...
  private static tryParseLenientStructuredResponse(rawText: string): StructuredResponse | null {
    ...
    const executions: StructuredExecution[] = [];
  ...
  private static readJsonLikeString(text: string, openingQuoteIndex: number) {
  ...
  private static readLooseJsonTailString(
    text: string,
  ...
  private static unescapeJsonLikeString(value: string) {
    return String(value || '')
      .replace(/\\\\/g, '\\')
      .replace(/\\"/g, '"')
      .replace(/\\n/g, '\n')
  ...
  private static findNextNonWhitespaceChar(text: string, startIndex: number) {
  ...
  private static extractBalancedJsonSegment(
    text: string,
    startIndex: number,
    openingChar: '{' | '[',
    closingChar: '}' | ']'
  ...
  private static compileExecutionsToCommands(executions: StructuredExecution[]): ExecutableCommand[] {
    const compiled: ExecutableCommand[] = [];
  ...
  private static normalizeExecutionItem(value: any): ExecutableCommand | null {
  ...
  private static rawSourceToCommand(source: string, language?: string): ExecutableCommand | null {
  ...
  private static pluginCallToCommand(value: any): ExecutableCommand | null {
    if (!this.isPluginCallExecution(value)) {
      return null;
    }
  ...
  private static isPluginCallExecution(value: any): value is PluginCallExecution {
    ...
    return ['Workspace3D.CreatePart', 'ScriptManager.Create', 'AssetImporter.InsertById', 'InstanceManager.Create', 'InstanceManager.SetProperty'].includes(value.api);
  }

  private static logStructuredResponseDebug(stage: string, response: StructuredResponse | null | undefined) {
    this.logDebug(stage, {
  ...
  private static logExecutableCommandsDebug(stage: string, commands: ExecutableCommand[]) {
    this.logDebug(stage, {
  ...
  private static previewText(value: string, max = 200) {
    const normalized = String(value || '').replace(/\r/g, '\\r').replace(/\n/g, '\\n');
    if (normalized.length <= max) return normalized;
    return `${normalized.slice(0, max)}...`;
  }

  private static logDebug(stage: string, payload: Record<string, any>) {
    try {
      console.info(`[ai-debug] ${stage}`, payload);
    } catch {}
  }

  private static commandToPluginCall(command: ExecutableCommand) {
    if (command.action === 'CreatePart') {
      return {
        type: 'plugin_call',
        api: 'Workspace3D.CreatePart',
        args: {
          shape: command.payload.shape,
          position: command.payload.position,
          size: command.payload.size,
          properties: command.payload.properties || {},
        },
      };
    }

    if (command.action === 'CreateScript') {
      return {
        type: 'plugin_call',
        api: 'ScriptManager.Create',
        args: {
          scriptType: command.payload.scriptType,
    ...
    if (command.action === 'CreateInstance') {
      return {
        type: 'plugin_call',
        api: 'InstanceManager.Create',
        args: {
          className: command.payload.className,
          parent: command.payload.parent,
          name: command.payload.name,
          properties: command.payload.properties || {},
        },
    ...
    if (command.action === 'SetInstanceProperty') {
      return {
        type: 'plugin_call',
        api: 'InstanceManager.SetProperty',
        args: {
    ...
    if (command.action === 'RunLuau') {
      return {
        type: 'free_command',
        language: command.payload.language || 'luau',
        source: command.payload.source || '',
      };
    }
  ...
  private static extractJsonArrayOrObject(rawText: string) {
    const trimmed = String(rawText || '').trim();
    if (
